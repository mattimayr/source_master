 ...
 */
public class EBEsSurrogateMethod2 extends Problem {
  /**
   * Constructor.
   * Creates a default instance of the EBEs problem using Surrogates.
   * @param solutionType The solution type must "Real" or "BinaryReal".
   */
	
	public EBEsSurrogateMethod2(SolutionType solutionType) {
		solutionType_ = solutionType;
	}
	
	public EBEsSurrogateMethod2(SolutionType solutionType, int maxEvaluations) {
		solutionType_ = solutionType;
		maxEvaluations_ = maxEvaluations;
	}
	
	public EBEsSurrogateMethod2(String solutionType) throws ClassNotFoundException {
	...
	public EBEsSurrogateMethod2(String solutionType, int maxEvaluations) throws ClassNotFoundException {
	  if (solutionType.compareTo("BinaryReal") == 0)
	    solutionType_ = new BinaryRealSolutionType(this) ;
	  else if (solutionType.compareTo("Real") == 0)
	    solutionType_ = new RealSolutionType(this) ;
	...
	private Surrogate surrogate1;
	
	public Surrogate getSurrogate1() {
		return surrogate1;
	}

	public void setSurrogate1(Surrogate surrogate1) {
		this.surrogate1 = surrogate1;
	}

	public int getRealInitialCounter() {
		return realInitialCounter;
	}

	public void setRealInitialCounter(int realInitialCounter) {
		this.realInitialCounter = realInitialCounter;
	}
	private Surrogate surrogate2;
	
	public Surrogate getSurrogate2() {
		return surrogate2;
	}

	public void setSurrogate2(Surrogate surrogate2) {
		this.surrogate2 = surrogate2;
	}
	
	private int modelComputeCounter;
	
	public int getModelComputeCounter() {
		return modelComputeCounter;
	}

	public void setModelComputeCounter(int computeCounter) {
		this.modelComputeCounter = computeCounter;
	}
	
	private int realComputeCounter;
	
	public int getRealComputeCounter() {
		return realComputeCounter;
	}

	public void setRealComputeCounter(int realComputeCounter) {
		this.realComputeCounter = realComputeCounter;
	}
	
	private int modelInitalCounter;
 ...
 */
  ...
  public void EBEsInitialize()  {
    ...
    problemName_= "EBEsSurrogate";
    numberOfEval_ = 1;

    //String file = EBEsReadProblems() + ".ebe";
    String file = "Bridge_Stayed_Cable_2Col_33G_133N_221B_93ZXY.ebe";
//    String file = "Mobile_Bridge_25N_35B_8G_16OrdZXY.ebe";
    try {
      // read file topology structural
    	EBEsReadDataFile(file);
    } catch (JMException ex) {
      Logger.getLogger(EBEsSurrogateMethod2.class.getName()).log(Level.SEVERE, null, ex);
    }
  ...
  /**
   * Evaluates OF1 (structure total weight)
   * @throws JMException 
   */
  private double evaluateOF1() throws JMException {
  	EBEsCalculus(); //  metodo matricial de la rigidez para estructuras espaciales (3D)
  	double fx = 0;
  	for(int ba=0; ba<numberOfElements_; ba++){
      int idx =(int)Element_[ba][INDEX_];
      fx +=Groups_[idx][AREA]*Element_[ba][L_]*Groups_[idx][DENSITY];
  	}
  	return fx;
  } // end evaluate OF1
  
  /**
   * Evaluates OF2 (maximize displacement nodes)
   * @throws JMException 
   */
  private double evaluateOF2() throws JMException {
  	EBEsCalculus(); //  metodo matricial de la rigidez para estructuras espaciales (3D)
  	double fx = 0;
  	int hi=0;
      ...
      fx += Math.sqrt(Math.pow(xn,2.0)+Math.pow(yn,2.0)+Math.pow(zn,2.0));
  	}
  	return fx;	
  } // end evaluate OF2
  
  /**
   * Evaluates OF3 (strain residual minimun)
   * @throws JMException 
   */
  private double evaluateOF3() throws JMException {
  	EBEsCalculus(); //  metodo matricial de la rigidez para estructuras espaciales (3D)
  	double fx = 0;
  	int hi = 0;
  	fx = StrainResidualMin_[hi]+StrainResidualMax_[hi];
  	return fx;
  } // end evaluate OF3

  /**
   * Evaluates a solution using surrogates 
   ...
   */
	...
	private void evaluateAndSet(Solution solution) throws JMException {
		switch(selectedOF) {
		case 12 : {
			solution.setObjective(0, evaluateOF1());
			solution.setObjective(1, evaluateOF2());
		} 
		break;
		case 13 : {
			solution.setObjective(0, evaluateOF1());
			solution.setObjective(1, evaluateOF3());
		}
		break;
		case 23 : {
			solution.setObjective(0, evaluateOF2());
			solution.setObjective(1, evaluateOF3());
		}
		break;
		case 123: {
			solution.setObjective(0, evaluateOF1());
			solution.setObjective(1, evaluateOF2());
			solution.setObjective(2, evaluateOF3());
		}
		break;
	}
}
